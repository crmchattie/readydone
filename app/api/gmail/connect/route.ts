...
export async function GET(request: Request) {
  try {
    ...
    const stateParam = referrer.includes('settings') ? 'settings' : 'home';
    ...
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/settings?error=Failed+to+initiate+OAuth+flow`);
  }
} 